...
From all these constraints, we deduce some possible candidates, to wit a pipeline of Incremental-$\Delta\Sigma$ with one last sub-ADC of any kind, an Extended-Range followed by a last sub-ADC of any kind. These maximize the robustness and benefit from the noise shaping of the $\Delta\Sigma$ modulation with a limited sensitivity to analog defects with few components.
...
\textbf{\textcolor{red}{principle and the basic choice}}
...
\textbf{\textcolor{red}{solution 1: 3-levels quantizer}}
...
\textbf{\textcolor{red}{error minimization}}
...
\textbf{\textcolor{red}{error minimization: threshold voltages tuning}}
...
\textbf{\textcolor{red}{solution 2: input feedforward}}
...
\textbf{\textcolor{red}{resolution and strategy}}
...
\textbf{\textcolor{red}{parasitic insensitive}}
...
\textbf{\textcolor{red}{integrator existing modes}}
...
\textbf{\textcolor{red}{Reduction of the INL without calibration by the ratio $C_S/C_I$}}
...
\textbf{\textcolor{red}{switches noise}}
...
\textbf{\textcolor{red}{channel resistance and settling time}}
...
\textbf{\textcolor{red}{charge injection}}
...
\textbf{\textcolor{red}{clock feedthough}}
...
Henceforth, from the allowed tolerance on the mismatch from the ideal values, the area of the unit capacitance can be found according to the Pelgrom modelling of capacitance variation in the design kit: \(\Delta C_0 \propto A_{c_{\rm MIM}}/\sqrt{WL} \) with \(C_0 \propto WL\times C_{\rm MIM}(fF/\mu m^2)\). In consequence, the unit capacitance is chosen to be 50 fF.
...
\begin{figure}[htp]
	...
	\end{subfigure}
	\caption{digital driver to ensure PMOS and NMOS signal transition crossing around half the power supply voltage}
...
\textbf{\textcolor{red}{limitations: analog error amplified and residue}}
...
\textbf{\textcolor{red}{limitations: resolution enhancement with interstage gain}}
...
\textbf{\textcolor{red}{limitations: resolution enhancement and threshold change}}
...
\textbf{\textcolor{red}{solutions: residue constrained with a 1.5-bit quantizer}}
...
\textbf{\textcolor{red}{inter stage gain}}
...
\textbf{\textcolor{red}{feedback gain}}
...
\textbf{\textcolor{red}{optimization of ratio}}
...
\textbf{\textcolor{red}{define what is reused in the digital}}
...
\textbf{\textcolor{red}{the problematic of non overlapping without using clock phases}}
...
\textbf{\textcolor{red}{solution 3: the one kept}}
...
\end{figure}
\textbf{\textcolor{red}{position of the attenutation capacitor to minimize the load}}
...
\textbf{\textcolor{red}{voltages generated and scale}}
By application of superposition, the output voltage \(V_r\) is given by equation~(\ref{eqn:output_voltage_dac_sar}) where \(C_i\) the capacitance connected to the voltage \(b_i V_{\rm ref}\) of the aforementioned figure. A consequence of the attenuation capacitor is the difficult implementation to have both a binary scale and \(C_a\) a multiple integer of the unit capacitance \(C_0\).
...
\textbf{\textcolor{red}{second stage decision error}}
...
\textbf{\textcolor{red}{attenuation capacitor rounding for redundancy}}
...
\textbf{\textcolor{red}{unit capacitor sizing procedure}}
...
\textbf{\textcolor{red}{matching error contribution}}
...
\textbf{\textcolor{red}{noise estimation}}
...
The sequencer resulting is depicted in \figurename~\ref{fig:sar-async-sequencer} to generate the phase in one hot encoding. In front of the first DFF allow continuous conversion for a start signal hold to zero. Therefore, the digital sequencer can be halted and resume the conversion process without a reset.
...
\textbf{\textcolor{red}{comparator clock generation}}
...
\textbf{\textcolor{red}{switches design}}
...
To not introduce settling error the positive reference voltage is connected to capacitors via low-vt pmos transistors, the negative reference voltage by low-vt nmos transistors, and the input via transmission gates whose pmos is 3.5 times bigger than nmos.

As only one switch is active at a time: the transmission gate only during the sampling clock cycle, and either the pmos or the nmos; 4 wires are required. 
...
In this section, the term simulation refers first to the Matlab simulation of the ADC\@. The Matlab simulation considers the two key limitations of the OTA, to wit,the finite gain and the unity gain frequency.
...
Therefore, based on this high-level spice simulations we deduce the minimum performances of the OTA without slewing.

Keeping these values, we set the Gain for 80 dB and the Unity-Gain-Frequency to 600 MHz. Without offset we get a resolution of 11.25-bits and 13.48-bits for respectively an oversampling ratio of 5 and 6.
...
For an OSR=5, we also represent the Differential Non-Linearity (DNL) and the Integral-Non-Linearity (INL) of the ADC where the expected resolution is 11.3-bits. So far, we were able to extract the error for an ideal reconstruction of the transfer function which is steps of binary word weighted by real values. As the number of bits produce exceed twelve, we can consider the weighted output code to be ideal. In case we quantize the weighted output code to a 12-bits one, the length of steps and the error are affected. So, we quantize the weighted output code to twelve bits and look at non-linearities of the ADC. Either with offsets or without offset, the non linearity does not exceed a 11-bits LSB\@. And this with only an offset correction based on the averaging of the output codes.
...
\begin{figure}[htp]
	...
	\begin{subfigure}[b]{0.8\textwidth}
		...
		\label{fig:adc-osr6-error-offset}
	\end{subfigure}
	\caption{DNL and INL of the proposed converter under large offset}
	\label{fig:adc-osr-error-offset}
\end{figure}
