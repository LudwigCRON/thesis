...
In addition to that, and without considering contact losses and recombination, charges also diffuse in the substrate. This \textcolor{blue}{substrate-current} depends on the electric field imposed by the MOSFET use case. In the case of a NMOS transistor as in \figurename~\ref{fig:soi-mos-leakage}a, the leakage current flow is either in one of the following configuration:
\begin{itemize}
	\item[--] $V_{\rm B} = V_{\rm S} < V_{\rm D}$: no charges flows in the intrinsic pn junction at the source but a reverse leakage current exists in the pn-junction at the drain.
	\item[--] $V_{\rm S} < V_{\rm B} < V_{\rm D}$: the reverse current in the pn-junction at the drain is smaller, and a forward leakage current occurs in the pn-jonction at the source.
	\item[--] $V_{\rm S} < V_{\rm D} < V_{\rm B}$: both pn-junction at the source and at the drain are forward biased. The current flows from the bulk to the each contact.
\end{itemize}

while for the PMOS transistor,
\begin{itemize}
	\item[--] $V_{\rm B} = V_{\rm S} < V_{\rm D}$: no charges flows in the intrinsic pn junction at the source but a forward leakage current exists in the pn-junction at the drain.
	\item[--] $V_{\rm S} < V_{\rm B} < V_{\rm D}$: the reverse current in the pn-junction at the source increases, and a forward leakage current occurs in the pn-jonction at the drain decrease.
...
Based on the Shockley equation, the forward and reverse current of the diode is defined by equation~(\ref{eqn:shockley-diode-leakage}) where $L_n (L_p)$ represents the equivalent height of the n-block (p-block), while $D_n (D_p)$ is the diffusion coefficients of this block, and $n_i$ is the intrinsic carrier concentation.
\begin{equation}
